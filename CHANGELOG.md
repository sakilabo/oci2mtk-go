...
- Move the command package to `cmd/oci2mtk` so `go install` creates the expected `oci2mtk` command instead of `oci2mtk-go`.

## [1.2.1] - 2026-06-30

### Changed

- Set the Go module path to `github.com/sakilabo/oci2mtk-go` to support installation with `go install`.

### Added

- Document installation from GitHub Releases and with `go install`.
